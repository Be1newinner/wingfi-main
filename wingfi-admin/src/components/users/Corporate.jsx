...
const Corporate = () => {
  ...
  const { data: userData, loading, error } = useSelector(selectCorporateUser);
  const [expandedRows, setExpandedRows] = useState([]);
  const limit = 10;
  const [page, setPage] = useState(parseInt(searchParams.get("page")) || 1); // Get page from URL or default to 1

  useEffect(() => {
    dispatch(userCorporateRequest({ page, limit }));
  }, [dispatch, page, limit]);
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0) {
      setPage(newPage);
      navigate(`/users/corporates?page=${newPage}`); // Update URL
    }
  };
  ...
  return (
    <div className="w-full flex justify-start flex-col h-full overflow-scroll no-scrollbar">
      <div className="p-5">
      <CommonHeaderRoute title="Corporates" userData={userData.data} />
      </div>
