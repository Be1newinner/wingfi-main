...
import { selectTrainService } from "../../redux/selectors/serviceSelector";
import { serviceTrainRequest } from "../../redux/reducers/serviceReducer";
...
const Train = () => {
  ...
  const { data: userData, loading, error } = useSelector(selectTrainService);
  const [expandedRows, setExpandedRows] = useState([]);
  const limit = 10;
  const [page, setPage] = useState(parseInt(searchParams.get("page")) || 1);

  useEffect(() => {
    dispatch(serviceTrainRequest({ page, limit }));
  }, [dispatch, page, limit]);

  const handlePageChange = (newPage) => {
    if (newPage > 0) {
      setPage(newPage);
      navigate(`/service/train?page=${newPage}`);
    }
  };
  ...
  return (
    <div className="w-full flex justify-center flex-col h-full overflow-scroll no-scrollbar">
      <div className="flex justify-between items-center mb-5 p-5">
        <h1 className="text-2xl font-bold">Train Service</h1>
        <button
